...
def run_model(config):
    ...
    algo_hp = str(config.commit_id[0:7]) + '_' \
              ...
              + str(config.architecture.get('batch_norm', 'Y')) \
              ...
              + str(config.hyperparams.get('weight_decay', 0)) + '_' \
    ...
    for i in tqdm(range(len(dataset)), desc="Pre-process"):
        ...
        dataset.graph_lists[i] = basis_transform(g, basis=basis, epsilon=epsilon, power=power,
                                                 degs=config.get('degs', []),
                                                 edgehop=config.get('edgehop', None),
                                                 basis_norm=(True if config.get('basis_norm', 'N') == 'Y' else False))
    ...
    for fold in range(config.num_folds):
        ...
        sel_epoch_idx = np.argmax(avg_test_accs)
        sel_test_acc = np.max(avg_test_accs)
        sel_test_acc_std = std_test_accs[sel_epoch_idx]
        ...
        print('--------')
        print('Best Test Acc:   ' + sel_test_with_std + ', Epoch: ' + str(sel_epoch_idx + 1))
